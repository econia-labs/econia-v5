...
base and quote they provide.

In the special case that they provide only base, price initializes to $p_l$ and their
contribution is effectively $b_{r, c}$. If only quote, $p_h$ and $q_{r, c}$.

Define two coefficients $C_b$ and $C_q$ per equations
\ref{eqn:clamm-endpoint-coefficients-b} and \ref{eqn:clamm-endpoint-coefficients-q}.

\begin{align} \label{eqn:clamm-endpoint-coefficients-b}
  b_{v, f} &= C_b b_{r, c} \nonumber \\
  C_b &= \frac{b_{v, f}}{b_{r, c}}
\end{align}

\begin{align} \label{eqn:clamm-endpoint-coefficients-q}
  q_{v, f} &= C_q q_{r, c} \nonumber \\
  C_q &= \frac{q_{v, f}}{q_{r, c}}
\end{align}
...
\begin{align}
  ...
  p_l &= \frac{q_{v, f}}{b_{r, c} + {b_{v, f}}} \nonumber \\
  p_l(b_{r, c} + b_{v, f}) &= q_{v, f} \nonumber \\
\end{align}

\begin{align}
  p_h &= \frac{q_v}{b_v}|_{p_h} \nonumber \\
  p_h &= \frac{q_{r, c} + q_{v, f}}{b_{v, f}} \nonumber \\
  b_{v, f} &= \frac{q_{r, c} + q_{v, f}}{p_h}
\end{align}

\begin{align}
  b_v q_v |_{p_l} &= b_v q_v |_{p_h} \nonumber \\
  q_{v, f} (b_{r, c} + {b_{v, f}}) &=  b_{v, f} (q_{r, c} + q_{v, f}) \nonumber \\
  p_l (b_{r, c} + {b_{v, f}})^2 &=  \frac{(q_{r, c} + q_{v, f})^2}{p_h} \nonumber \\
  p_h p_l &=  \frac{(q_{r, c} + q_{v, f})^2}{(b_{r, c} + {b_{v, f}})^2}
\end{align}
