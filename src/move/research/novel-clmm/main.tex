...
Let the first \gls{lp} provision liquidity $L_v$. Per the constant product invariant:
...
\begin{align}
  b_{v, c} p_l b_{v, c} &= L_v^2 \nonumber \\
  b_{v, c}^2 p_l &= L_v^2 \nonumber \\
  b_{v, c} &= \frac{L_v}{\sqrt{p_l}}
\end{align}

Substitute (\ref{eqn:first-lp-ph}) into (\ref{eqn:first-lp-invariant-2}):

\begin{align} \label{eqn:first-lp-invariant-2}
  b_{v, f} p_h b_{v, f} &= L_v^2 \nonumber \\
  b_{v, f}^2 p_h &= L_v^2 \nonumber \\
  b_{v, f} &= \frac{L_v}{\sqrt{p_h}} \\
\end{align}
