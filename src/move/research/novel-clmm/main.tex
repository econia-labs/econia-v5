...
\begin{align} \label{eqn:first-lp-invariant-3}
  (b_r + \frac{L}{\sqrt{p_h}})(q_r + L \sqrt{p_l}) = L^2 \nonumber \\
\end{align}

Solve (\ref{eqn:first-lp-invariant-3}) for $b_r$ in terms of $q_r$ and $L$:

\begin{align}
  b_r + \frac{L}{\sqrt{p_h}} &= \frac{L^2}{q_r + L \sqrt{p_l}} \nonumber \\
  b_r &= \frac{L^2}{q_r + L \sqrt{p_l}} - \frac{L}{\sqrt{p_h}} \nonumber \\
  b_r &= \frac{L^2 \sqrt{p_h}}{\sqrt{p_h}(q_r + L \sqrt{p_l})}
  - \frac{L(q_r + L \sqrt{p_l})}{\sqrt{p_h}(q_r + L \sqrt{p_l})}\nonumber \\
\end{align}
